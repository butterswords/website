# Books to Organize

This is a temporary staging doc, not published content (it lives under
`docs/`, outside `content/`, so Hugo won't render it). It's everything
that used to live below "Nonfiction" on `/bookshelf/`, pulled out so
the page itself could be rewritten as an intro + filterable grid.

Each entry below should eventually become its own page:

```
hugo new --kind book_review_template content/bookshelf/book-slug.md
```

...with `category` set to one of: Formative, Inspirations, In Progress,
Backlog, Recommendations, Reviews. The old section headings are kept
here only as a starting hint for that mapping — they don't line up
1:1 with the new categories, so don't assume "Critical Reads" ==
"Reviews" without thinking about each book individually.

Delete this file once everything below has been migrated.

---

# Nonfiction

## My Publications (Chronologically)
* [*Position: In-House Evaluation Is Not Enough. Towards Robust Third-Party Evaluation and Flaw Disclosure for General-Purpose AI*](https://openreview.net/forum?id=ACzL62Jp4E), 2025
* [*Exploring Subjective Notions of Explainability through Counterfactual Visualization of Sentiment Analysis*](https://www.computer.org/csdl/proceedings-article/beliv/2024/284600a015/22a0FIBygUM), 2024
* [*Book Review: Big Data, Big Design: Why Designers Should Care about Artificial Intelligence*](https://www.tandfonline.com/eprint/WNEPYR3THRKVM8TDQNAW/full?target=10.1080/17547075.2023.2213523), 2023

## Critical Reads
* [*Sand Talk; How Indigenous Thinking Can Save the World*](https://www.harpercollins.com/products/sand-talk-tyson-yunkaporta), Tyson Yunkaporta
* [*Thinking in Systems*](https://donellameadows.org/systems-thinking-book-sale/), Donella Meadows
* [*An Introduction to Systems Safety Engineering*](https://mitpress.mit.edu/9780262546881/an-introduction-to-system-safety-engineering/), Nancy Leveson
* [*The Age of Surveillance Capitalism*](https://shoshanazuboff.com/book/about/), Shoshana Zuboff
* [*This is How they Tell Me the World Ends: The Cyberweapons Arms Race*](https://thisishowtheytellmetheworldends.com/), Nicole Perlroth
* [*Race after Technology: Abolinist Tools for the New Jim Code*](https://www.ruhabenjamin.com/race-after-technology), Ruha Benjamin
* [*Algorithms of Oppression: How Search Engines Reinforce Racism*](https://safiyaunoble.com/research-writing/), Safiya Umoja Noble
* [*Weapons of Math Destruction: How Big Data Increases Inequality and Threatens Democracy*](https://mathbabe.org/), Cathy O'Neil
* [*Design Justice: Community-Led Practicies to Build the Worlds We Need*](https://design-justice.pubpub.org/), Sasha Costanza-Chock
* [*The Master Algorithm: How the Quest for the Ultimate Learning Machine Will Remake Our World*](https://en.wikipedia.org/wiki/The_Master_Algorithm), Pedro Domingos

## Works that have inspired me
* [*The Visual Display of Quantitative Information*](https://www.edwardtufte.com/tufte/books_vdqi), Edward Tufte
* [*Sickness unto Death*](https://en.wikipedia.org/wiki/The_Sickness_unto_Death), Søren Kierkegaard
* [*I and Thou*](https://en.wikipedia.org/wiki/I_and_Thou), Martin Buber
* *Existentialism and Human Emotions*, John Paul Sartre

## Backlog
* [*All Too Human*](https://www.cambridge.org/core/books/abs/all-too-human/all-too-human/19DD407A3539FD8FFF3BB9F4D64B9794), Anne McLaughlin
* [*Data Feminism*](https://mitpress.mit.edu/books/data-feminism), Catherine D'Ignazio and Lauren F. Klein
* [*The Costs of Connection*](https://colonizedbydata.com/), Nick Couldry and Ulises A Mejias
* [*Value Sensitive Design: Shaping Technology with Moral Imagination*](https://direct.mit.edu/books/book/4328/Value-Sensitive-DesignShaping-Technology-with), Batya Friedman and David G. Henry


## Noteworthy Mentions
* [*The Tyranny of Merit: What's Become of the Common Good?*](https://us.macmillan.com/books/9780374289980/thetyrannyofmerit), Michael J. Sandel
* [*The Signal and the Noise: Why So Many Predictions Fail - but Some Don't*](https://www.penguinrandomhouse.com/books/305826/the-signal-and-the-noise-by-nate-silver/), Nate Silver
